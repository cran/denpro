profeval<-function(tr,
Q=NULL,frekv=NULL,cvol=TRUE,ccen=TRUE,cfre=FALSE)
{

d<-dim(tr$upp)[2]
step<-stepcalc.denpro(tr$support,tr$N)    #  n+1 <-> n
#step<-matrix(0,d,1)
#for (i in 1:d){
#   step[i]<-(tr$support[2*i]-tr$support[2*i-1])/(tr$N[i]+1)
#}
...
rec<-matrix(0,2*d,1)
for (j in 1:d){
   rec[2*j-1]<-tr$support[2*j-1]+tr$low[1,j]*step[j]
   rec[2*j]<-  tr$support[2*j-1]+tr$upp[1,j]*step[j]
}
center=t(cenone(rec))
...
inval[2:(nodenumOfTree+1)]<-tr$split
...
if (cvol){
#  volume<-cvolumbag(component=component,AtomlistAtom=AtomlistAtom,AtomlistNext=AtomlistNext,low=tr$low,upp=tr$upp,steppi=step)
