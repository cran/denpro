decomgra2<-function(lsets,levels,links,radius,alkublokki,blokki){
#Makes density tree,  edellyttaa jarjestyksen ???????
#difference to decom: diff links to diff lsets, transform radius
#
...
#
levnum<-length(lsets[,1])      #lsets:n (tieto:n) rivien maara 
...
newradius<-matrix(0,atomnum,alkublokki)
#
...
if (levnum>1){  while (pinind>=1){
  ...
  if (sum(partlevset-levset)==0){  
      ...
      rindeksit<-change(levset)
      curlinks<-links[(atomnum*(levind-1)+1):(atomnum*levind),]
      kumu<-declev(rindeksit,curlinks,atomnum) #jaet tasoj. osa osiin
      if (dim(t(kumu))[1]==1) koko<-1 else koko<-length(kumu[,1]) #osien lkm
      if ((efek+koko)>curblokki){   
        ...
        newradius<-blokitus2(newradius,blokki)
      ...
      newradius[,(efek+1):(efek+koko)]<-radius[,levind+1] 
      efek<-efek+koko
      if (levind+1<levnum){ #jos ei olla korkeimmalla tasolla,laita pinoon
...
newradius<-newradius[,1:efek]
return(list(lsets=newlsets,levels=newlevels,parents=parents,radius=newradius))
}
