graph.matrix<-function(dendat,tt=NULL,permu=seq(1:dim(dendat)[1]),
...
if (is.null(tt)){
   ...
   for (j in 1:d){
      for (i in 1:n){
       ...
       text(origins[j],ystart,as.character(round(means[j],digits=2)))
