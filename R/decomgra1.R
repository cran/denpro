decomgra1<-function(lsets,levels,links,radius,alkublokki,blokki){
...
newradius<-matrix(0,alkublokki,1)
#
...
if (levnum>1){  while (pinind>=1){
  ...
  if (sum(partlevset-levset)==0){  
      ...
      if ((efek+koko)>curblokki){   
        ...
        newradius<-blokitus(newradius,blokki)
